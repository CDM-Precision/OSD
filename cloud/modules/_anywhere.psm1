...
function osdcloud-InstallPowerShellModule {
    ...
    $zipPath = "$env:TEMP\osd.zip"
    $extractPath = "$env:TEMP\$CDMModuleName"
    $modulePath = "$env:ProgramFiles\WindowsPowerShell\Modules\$CDMModuleName"

    # download zip
    Invoke-WebRequest -UseBasicParsing -Uri 'https://github.com/CDM-Precision/OSD/archive/refs/heads/master.zip' -OutFile $zipPath
